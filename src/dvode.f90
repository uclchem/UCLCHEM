...
!
! 4 June 2013 by KE Niemeyer: made safe for use with OpenMP 
! through THREADPRIVATE directives
!
...
! JACSPDB arrays and parameters.
  ...
  !$OMP THREADPRIVATE(INDROWDS, INDCOLDS, NGRPDS, IPNTRDS, JPNTRDS, IWADS, IWKDS, &
  !$OMP& IOPTDS, YSCALEDS, WKDS, FACDS, U125, U325)
...
! .. Local Arrays ..
      ...
      !$OMP& HMXI, HNEW, HSCAL, HU, MEPS, MRESID, MRMIN, PRL1, RC, RESID, RL1, &
      !$OMP& RMIN, SETH, T0ST, THEMAX, TLAST, TN, TOL, TOL1, TOUTC, UMAX, UROUND, &
      !$OMP& U_PIVOT, X2, WM1, WM2)
      
      !$OMP THREADPRIVATE (ADDTOJA, ADDTONNZ, CONSECUTIVE_CFAILS, CONSECUTIVE_EFAILS, &
      !$OMP& ELBOW_ROOM, IADIM, IANPIV, IAVPIV, ICF, ICNCP, IFAIL, IMAX, IMIN, INEWJ, &
      !$OMP& INIT, IPUP, IRANK, IRFND, IRNCP, ISTART, ISTATC, ITASKC, JADIM, JCUR, JMIN, &
      !$OMP& JSTART, JSV, KFLAG, KOUNTL, KUTH, L, LARGE, LAST, LENIGP, LICN_ALL, LIRN_ALL, &
      !$OMP& LIW, LIWM, LMAX, LOCJS, LP, LRW, LWM, LWMDIM, LWMTEMP, LYH, LYHTEMP, MANPIV, &
      !$OMP& MAPIV, MAXG, MAXIT, MAXORD, MB28, MB48, METH, MICN, MICNCP, MINICN, MINIRN, MIRANK)
      
      !$OMP THREADPRIVATE(MIRN, MIRNCP, MITER, MLP, MOSS, MP, MSBG, MSBJ, MXHNIL, MXSTEP, N, NZB, NCFN, &
      !$OMP& NDROP, NDROP1, NDX, NETF, NEWH, NEWQ, NFE, NGC, NGE, NGP, NHNIL, NJE, NLP, &
      !$OMP& NLU, NNI, NNZ, NOITER, NQ, NQNYH, NQU, NQWAIT, NSLG, NSLJ, NSLP, NSRCH, NSRCH1, &
      !$OMP& NST, NSUBS, NSUPS, NUM, NUMNZ, NYH, NZ_ALL, NZ_SWAG, PREVIOUS_MAXORD, WPD, WPS, &
      !$OMP& MA28AD_CALLS, MA28BD_CALLS, MA28CD_CALLS, MC19AD_CALLS, MAX_MINIRN, MAX_MINICN, &
      !$OMP& MAX_NNZ, BNGRP)
      
      !$OMP THREADPRIVATE (ABORT, ABORT1, ABORT2, ABORT3, ABORTA, ABORTB, ALLOW_DEFAULT_TOLS, &
      !$OMP& BUILD_IAJA, BOUNDS, CHANGED_ACOR, GROW, IAJA_CALLED, J_HAS_BEEN_COMPUTED, J_IS_CONSTANT, &
      !$OMP& LBIG, LBIG1, LBLOCK, MA48_WAS_USED, OK_TO_CALL_MA28, SUBS, SUPS, OPTS_CALLED, &
      !$OMP& REDO_PIVOT_SEQUENCE, SCALE_MATRIX, SPARSE, USE_FAST_FACTOR, YMAXWARN)
      
      !$OMP THREADPRIVATE (ACOR, CSCALEX, EWT, FPTEMP, FTEMP, FTEMP1, G0, G1, GX, JMAT, &
      !$OMP& LB, PMAT, RSCALEX, RWORK, SAVF, UB, WM, WMTEMP, WSCALEX, YHNQP2, YHTEMP, YMAX, &
      !$OMP& YNNEG, YTEMP, DTEMP, EL, RUSER, TAU, TQ)
      
      !$OMP THREADPRIVATE (BIGP, BJGP, IA, IAB, IAN, ICN, IDX, IGP, IKEEP28, IW28, IWORK, JA, &
      !$OMP& JAB, JAN, JATEMP, JGP, JROOT, JVECT, SUBDS, SUPDS, IDISP, IUSER, LNPIV, LPIV, MORD)
