...
def run_makerates(configuration_file="user_settings.yaml", verbosity="INFO", write_files=True):
    ...
    reaction_files = [user_params["database_reaction_file"],user_params["custom_reaction_file"]]
    reaction_types = [user_params["database_reaction_type"],user_params["custom_reaction_type"]]
    species_file = user_params["species_file"]
    three_phase = user_params["three_phase"]

    # retrieve the network and the dropped reactions
    network, dropped_reactions = _get_network_from_files(reaction_files=reaction_files, reaction_types=reaction_types, species_file=species_file, three_phase=three_phase, return_dropped_reaction=write_files)
    ...
    if write_files:
        ...
        io.output_drops(dropped_reactions=dropped_reactions, user_output_dir=user_output_dir, write_files=write_files)
...
def get_network(path_to_input_file: Union[str, bytes, os.PathLike]=None,
    ...
    if (bool(path_to_input_file) and bool(path_to_species_file or path_to_reaction_file)):
        raise ValueError("Cannot have both an input Makerates config file and explicit paths to species + reaction files")
        
    if path_to_input_file:
        return(run_makerates(path_to_input_file, verbosity=verbosity, write_files=False))
    else:
        pass
        
    
def _get_network_from_files(species_file: Union[str, bytes, os.PathLike],
                            reaction_files: Union[str, bytes, os.PathLike],
                            reaction_types: str,
                            three_phase: bool,
                            ):
    ...
    for reaction_file, reaction_type in zip(reaction_files, reaction_types):
        ...
        reactions.append(temp_reactions)
        reactions.append(temp_dropped_reactions)
    # Create Network
    dropped_reactions=sum(dropped_reactions)
    network =  Network(species=species_list, reactions=sum(reactions), three_phase=three_phase, user_defined_bulk=user_defined_bulk)
