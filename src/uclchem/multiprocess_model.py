...
class AbstractModel(ABC):
    ...
    def load_from_dataset(cls, model_ds: xr.Dataset, debug: bool = False):
        obj = cls.__new__(cls)
        obj.param_dict = json.loads(model_ds["param_dict"].item())
        model_ds.__delitem__("param_dict")
    ...
    def __getattr__(self, key):
        if (key in ["debug", "_prev_handler", "_shm_desc", "_shm_handles", "_orig_sigint", "_proc_handle"] or
                key in object.__getattribute__(self,"__dict__")):
    ...
    def __setattr__(self, key, value):
        if key in ["param_dict", "_data", "debug", "_prev_handler", "_shm_desc", "_shm_handles", "_orig_sigint", "_proc_handle"]:
    ...
    def run(self):
        ...
        elif self.run_type in ["managed", "external"]:
    ...
    def _create_fortran_array(self):
        ...
        if not self.run_type == "local":
            ...
            self._shm_handles["chemical_abun_array"], self._shm_desc["chemical_abun_array"], self.chemical_abun_array = self._create_shared_memory_allocation(
                (self.timepoints + 1, self.param_dict["points"], n_species))
        else:
    ...
    def _create_rates_array(self):
        ...
        if not self.run_type == "local":
            self._shm_handles["rates_array"], self._shm_desc["rates_array"], self.rates_array = self._create_shared_memory_allocation(
                (self.timepoints + 1, self.param_dict["points"], n_reactions))
        else:
    ...
    def on_interrupt(self):
        ...
        self.save_model(file="./" + error_time + ".h5", name="interrupted", overwrite=True)
        return
    # /Signal Interrupt Catch
...
class Cloud(AbstractModel):
    ...
    def run_fortran(self):
        ...
        if self.debug:
            ...
            print(f"using "
                  f"dictionary={self.param_dict},"
                  f"outspeciesin={self.out_species},"
                  f"timepoints={self.timepoints},"
                  f"gridpoints={self.param_dict['points']},"
                  f"returnarray={True},"
                  f"returnrates={True},"
...
class GridModels:
    def __init__(self,
        ...
        self.max_workers = max_workers if max_workers < int(os.cpu_count()/2) else int(os.cpu_count()/2)
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as p:
            models_list = p.map(self.run_grid_model, list(grid_iter(full_parameters, list(self.parameters_to_grid.keys()), self.flat_grids)))
        self.models = {}
    ...
    def run_grid_model(self, init_kwargs):
        model_id = init_kwargs.pop("id")
        param_dict = {}
        tmp_list = list(init_kwargs.keys())
        for key in tmp_list:
            if key.lower() in default_param_dictionary:
                param_dict[key] = init_kwargs.pop(key)
        cls = REGISTRY.get(self.model_type)
        model = cls(param_dict=param_dict, **init_kwargs, run_type="local")
        return {f"{model_id}": model}

    def save_models(self, file_name, grid_name: str = 'grid_model', overwrite:bool = False):
        for k, v in self.models.items():
            v.save_model(file_name, f"{grid_name}_{k}", overwrite)
