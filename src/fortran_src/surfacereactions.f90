module SurfaceReactions
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  real(dp), parameter :: GAS_DUST_MASS_RATIO=100.0,GRAIN_RADIUS=1.0d-5, GRAIN_DENSITY = 3.0  ! Mass density of a dust grain
...
function reacProb(reacIndx, index1, index2, dustTemperature)
    ...
    if (reducedMass == MISSING_VALUE_FLOAT) then
...
function getDesorptionFractionFullCoverage(reacIndx, LHDESindex) result (desorptionFractionFullCoverage)
    ...
    if (useMinissaleIceChemdesEfficiency) then
        ...
        if (re1(reacIndx)==ngn.and.re2(reacIndx)==ngn) desorptionFractionFullCoverage = 0.5D0
        if ((re1(reacIndx)==ngo.and.re2(reacIndx)==nh) &
            &.or. (re1(reacIndx)== nh.and.re2(reacIndx)==ngo)) desorptionFractionFullCoverage = 0.3D0
        if ((re1(reacIndx)==ngoh.and.re2(reacIndx)==nh) &
            &.or. (re1(reacIndx)==nh.and.re2(reacIndx)==ngoh)) desorptionFractionFullCoverage = 0.25D0
        return
    end if
    ...
    if (chi * deltaEnthalpy - bindingEnergyDesorbingSpec < 0.0) then
        desorptionFractionFullCoverage = 0.0D0
    else
...
real(dp) function EncounterDesorptionRate(reacIndx,dustTemperature)
    ...
    if (EDEndothermicityFactor /= 0.0) then
        meetProb = meetProb &
