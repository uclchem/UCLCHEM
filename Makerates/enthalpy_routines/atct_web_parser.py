...
class ATCTParser:
    ...
    def parse_html_file(self, html_file_path: str | Path) -> pd.DataFrame | None:
        ...
        ValueError
            If the main data table could not be located, or no data was found in
            the table.
        RuntimeError
            If the parsing of the ATcT file failed.
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        -------
        pd.DataFrame
            DataFrame with unnecessary columns removed, and cleaned column names.
    ...
    def _clean_unicode_string(text: Any) -> str:
        ...
        ----------
        text : Any
            Object whose string representation is cleaned.
    ...
    def get_summary_stats(data: pd.DataFrame) -> dict[str, int]:
        ...
        -------
        dict[str, int]
            Dictionary with summary statistics (total_species, species_with_298k,
            species_with_0k, gas_phase, unique_phases).
    ...
    def validate_data(self, data: pd.DataFrame, min_species: int = 3000) -> None:
        ...
        data : pd.DataFrame
            Parsed ATCT data.
        min_species : int
            Minimum expected number of species. Defaults to ``3000``.

        Raises
        ------
        ValueError
            If ``data`` does not contain at least ``min_species`` entries or
            does not contain at least ``min_species`` entries with 298 K data.
...
def main() -> None:
    ...
    try:
        ...
        if data is None:
            print("❌ Error: parsing returned no data")
            sys.exit(1)
