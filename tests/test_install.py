...
def test_package_installation():
    ...
    result = subprocess.run("cd Makerates; python Makerates.py data/small_chemistry/user_settings.yaml; cd ../ ", shell=True, text=True, capture_output=True)
    
    assert result.returncode == 0, f"Installing an alternative network failed: \n{result.stdout}\n{result.stderr}"
    
    result = subprocess.run(install_command, shell=True, text=True, capture_output=True)
    
    # Check if the installation was successful
    assert result.returncode == 0, f"Package installation w/ small network failed:\n{result.stdout}\n{result.stderr}"
