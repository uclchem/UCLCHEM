...
for model in ["phase1","phase2","static"]:
	...
	#call read_uclchem. 
	time,dens,temp,abundances=read_uclchem("examples/"+model+"-full.dat",speciesNames)
	...
	axis,fig=plot_species(speciesNames,time,abundances,plotFile="examples/"+model+".png")

	#plot species returns the axis so we can further edit
	axis.set_xscale('log')
	axis.set_title(model)
	fig.savefig("examples/"+model+".png")
