...
desorption for all but the first few time steps. Turning it off doesn't affect the abundances but will speed up 
the solution.
...
### Checking the Result
With a successful run, we can check the output. We first load the file and check the abundance conservation, then we 
can plot it up. To demonstrate how these files can be read in the new class method, we recreate the `p_core` object 
but instead of passing `param_dict`, we use the argument `read_file` and point it to the full output file we previously 
wrote to.
'''

p_core = uclchem.model.PrestellarCore(temp_indx=3, max_temperature=300.0, read_file="../examples/test-output/phase2-full.dat")
...
using the abundances we just produced. Note that c-shock is the only model which returns an additional output in its 
result list. Shock models include an additional variable that can be accessed, the dissipation time. This can be 
...
cshock = uclchem.model.CShock(shock_vel=40, read_file="../examples/test-output/cshock.dat")
# dissipation_time = uclchem.utils.cshock_dissipation_time(shock_vel=40, initial_dens=param_dict["initialDens"])
...
UCLCHEM will not check that the initial density matches the density of the `abundLoadFile`. It may not always be a good 
idea to do this, but we should remember the initial abundances really are just a rough approximation.

By default, UCLCHEM uses 500 timepoints for a model, but this turns out not to be enough, which is why we increase the 
...
jshock = uclchem.model.JShock(shock_vel=shock_vel, read_file="../examples/test-output/jshock.dat")
...
Bear in mind that you can use `abundSaveFile` and `abundLoadFile` in the same model run. This lets you chain model 
runs together. For example, you could run a c-shock from a cloud model as we did here and then a j-shock with the 
c-shock's abundances as the initial abundances.
'''